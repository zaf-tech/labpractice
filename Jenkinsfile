    ...
    steps {
        script {
            ...
            waitUntil(timeout: 60, unit: 'SECONDS') { // Timeout after 1 minute
                try {
                    sh "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/home/jenkins/.ssh/known_hosts -o ConnectTimeout=10 ec2-user@${publicIp} 'exit 0'" // Try connecting
            ...
            sshagent(credentials: ['ec2-user']) {
                sh """
                    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/home/jenkins/.ssh/known_hosts ec2-user@${publicIp} << EOFSSH
                        #!/bin/bash
                        # ... your commands ...
EOFSSH
                """
